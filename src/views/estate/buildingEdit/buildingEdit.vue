  ...
  <div id="building-edit " class="estate-page">
    ...
    <div class="estate-main">
      <van-cell-group title="楼栋门牌">
        ...
        <plate-prefix-control @prefix-picker-confirm="prefixPickerConfirm"></plate-prefix-control>

        <van-field label="楼栋号:" v-model="plate.plateNumber" required input-align="right" placeholder="请输入楼栋门牌" />
        <plate-suffix-control @suffix-picker-confirm="suffixPickerConfirm"></plate-suffix-control>
        <!-- </van-row> -->
      </van-cell-group>
      ...
      <div v-if="!isAddnew(this)" v-show="this.$store.state.user.role == 'worker'">
        ...
        <van-button style=" height:40px;width:100%;margin-right:10px" @click="onUpdateBuildingToDb()" plain type="info"> 保存</van-button>
...
import { NavBar, Icon, Field, Button, Row, Col, Tabs, Tab, Cell, CellGroup, Switch, Stepper } from 'vant';
...
import { UpdateBuildingToDb } from './buildingEdit.addon.js';
...
export default {
  ...
  methods: {
    ...
    onUpdateBuildingToDb: function() {
      this.plate.plateDesc = this.computePlateDesc();
      this.plate.plateAlias = this.plate.plateDesc;
      this.objBuilding.buildingAlias = this.computePlateDesc();
      UpdateBuildingToDb(this);
    },
    ...
    suffixPickerConfirm: function(data) {
      this.objBuilding.buildingPlate.plateSuffixValue = data.text;
      this.objBuilding.buildingPlate.plateSuffixId = data.keyId;
      // this.plate.plateDesc = this.computePlateDesc();
      // this.plate.plateAlias = this.plate.plateDesc;
      // this.objBuilding.buildingAlias = this.computePlateDesc();
    },
    prefixPickerConfirm: function(data) {
      this.objBuilding.buildingPlate.platePrefixValue = data.text;
      this.objBuilding.buildingPlate.platePrefixId = data.keyId;
      // this.plate.plateDesc = this.computePlateDesc();
      // this.plate.plateAlias = this.plate.plateDesc;
      // this.objBuilding.buildingAlias = this.computePlateDesc();
    },

  ...
  <div id="room-plate-panel" style="height:400px">
    ...
    <div v-if="!loading" style="z-index:10">
      <van-row style="margin-top:10px;">
        ...
        <van-field label="门牌全称:" :value="this.plateDesc()" label-width="60px" input-align="right" />

        <van-field label="楼栋显示:" readonly input-align="right">
          <van-switch size="20" slot="right-icon" v-model="buildingPlateShow" style="margin-right:10px" />
        </van-field>
        <van-field label="单元显示:" input-align="right">
          <van-switch size="20" slot="right-icon" v-model="unitPlateShow" style="margin-right:10px" />
        </van-field>
        <van-field label="楼层显示:" input-align="right">
          <van-switch size="20" slot="right-icon" v-model="floorPlateShow" style="margin-right:10px" />
        </van-field>
        <van-field label="房间显示:" input-align="right">
          <van-switch size="20" slot="right-icon" v-model="roomPlateShow" style="margin-right:10px" />
        </van-field>
        <van-field label="保存设置:" input-align="right">
          <van-button style=" height:25px;line-height:25px;" type="info" round slot="right-icon" @click="onSaveClick()" block> 保存</van-button>
          />
        </van-field>
      </van-row>
    </div>
  ...
  created: async function() {
    ...
    if (roomId !== undefined) {
      try {
        ...
        this.division = res.data.result.province + '/' + res.data.result.city + '/' + res.data.result.district;

    ...
    <div class="my-skeleton ">
      ...
      <div class="my-skeleton-content">
        <div id="title" class="title">
          ...
          <div class="icon">
            <van-icon size="30" color="#409eff" class-prefix="my-icon" name="arrow-right" @click="onClickEstateModify" />
            <!-- <i class="van-icon van-icon-arrow" @click="onClickEstateModify" /> -->
          </div>
        </div>
        <div id="note" class="note">
          <span>用地单位：{{ this.estateInstance.owner }}</span>
          <van-col span="14"><span>审核状态：</span>
            <van-tag plain size="small" :color="getAuditColor()">{{
              getAuditText()
            }}</van-tag>
          </van-col>
        </div>
